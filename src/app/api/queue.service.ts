...
export class QueueService {
    ...
    getHistory(type: string = 'all'): Promise<IPagedResult<IQueuedTrack>> {
        var url = `${this._audioZoneService.getCurrentZoneSnapshot().path}/api/history?filter=${type}&start=0&take=50`;
    ...
    getAllQueuedTracks(): Promise<IQueuedTrack[]> {
        var url = `${this._audioZoneService.getCurrentZoneSnapshot().path}/api/Queue`;
    ...
    queueTrack(track: ITrack, comment: string = null) {
        var url = `${this._audioZoneService.getCurrentZoneSnapshot().path}/api/Queue/Enqueue/${track.MusicProvider.Identifier}/${track.Link}`;
